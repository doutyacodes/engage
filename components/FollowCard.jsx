...
const FollowCard = ({ item, user_id }) => {
        ...
        if(item.followed_user != user_id ){
            try {
                const response = await axios.get(
                  `${baseURL}/toggle-user-follow.php?followed_user=${user_id}&user_id=${item.followed_user}`
                );
  ...
  return (
    ...
    >
      ...
      <TouchableOpacity
        style={{ backgroundColor: item.followed_user != user_id ? "#0866ff" : "gray", padding: 8, borderRadius: 3 }}
        onPress={toggleFollow} 
      >
        <Text style={{ fontSize: hp(1.7), fontFamily: "raleway-bold", color: "white" }}>
