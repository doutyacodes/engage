...
const UserCommentStructure = ({
  ...
  const [heartActive, setHeartActive] = useState(item.already_liked ? item.already_liked : false);
  const [count, setCount] = useState(parseInt(item.user_comment_likes_count ? item.user_comment_likes_count : 0));
  const handleHeart =async () => {
    // Alert.alert(item.post_id)
    try {
      const response = await axios.get(
        `${baseURL}/toggle-user-comments.php?post_id=${item.post_id}&comment_author=${item.user_id}&comment_id=${item.id}&user_id=${user_id}`
      );
  ...
  const handleDelete = async (id) => {
    try {
      const response = await axios.get(
        `${baseURL}/delete-user-comment.php?id=${id}`
      );
      // console.log(response.data);
      if (response.status === 200) {
        Toast.show({
  ...
  const handleReport = async () => {
    try {
      const response = await axios.get(
        `${baseURL}/report-user-comment.php?comment_id=${item.id}&post_id=${item.post_id}&user_id=${item.user_id}&reported_user=${user_id}`
      );
  ...
  return (
    <View style={{ position: "relative" }}>
      <TouchableOpacity>
        <Text style={{ fontSize: 14, color: "#525252",fontStyle:item.reported =="no" ? "normal" : "italic" }}>{item.reported =="no" ? comment_text : "This comment is reported by you."}</Text>
      </TouchableOpacity>
      <View
        style={{
          flexDirection: "row",
...
export default UserCommentStructure;
