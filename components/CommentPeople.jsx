...
import CommentStructure from "./AppComponents/CommentStructure";
import ReplyStructure from "./AppComponents/ReplyStructure";
import Toast from "react-native-root-toast";
const CommentPeople = ({ route }) => {
    ...
    if (item) {
      try {
        const response = await axios.get(
          `${baseURL}/getComment.php?people_id=${item.people_data_id}&userId=${user_id}`
        );
                    ...
                    >
                      ...
                      <View style={{ marginLeft: "auto" }}>
                        <TouchableOpacity onPress={()=>{
                          showOptions == item.id ?setShowOptions(0) : setShowOptions(item.id)
                        }}>
                          <Entypo
