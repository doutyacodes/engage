...
TaskManager.defineTask(async ({ data, error }) => {
  ...
  console.log('Background task triggered, attempting foreground magnetometer read...');
  try {
...
function Testapge() {
  ...
  return (
    <View style={styles.container}>
      ...
      {magnetometerData && (
        <Text>
          x: {magnetometerData.x.toFixed(2)} μT, y: {magnetometerData.y.toFixed(2)} μT,
          z: {magnetometerData.z.toFixed(2)} μT
        </Text>
      )}
      <Button
        title={isBackgroundTaskRegistered ? 'Stop Background Task' : 'Start Background Task'}
        onPress={isBackgroundTaskRegistered ? handleStopBackgroundTask : handleStartBackgroundTask}
      />
      <Text style={styles.disclaimer}>
        * Background magnetometer access is currently not directly supported in Expo.
        * This example demonstrates triggering a background task, but reading may not work.
      </Text>
    </View>
  );
}
