  ...
  const startCounting = () => {
    ...
    const interval = setInterval(() => {
      ...
      if (elapsedSeconds >= durationAsNumber) {
        ...
        setMinutes(`${minutes}:${remainingSeconds < 10 ? '0' : ''}${remainingSeconds}`);
    ...
    if (cameraRef.current && audioPermission === true) {
      try {
        if (recording) {
          ...
          const isDurationValid = !isNaN(durationAsNumber) && durationAsNumber > 0;
          const recordParams = isDurationValid ? { maxDuration: durationAsNumber } : {};

          const { uri } = await cameraRef.current.startRecording(recordParams);
          setVideoSource(uri);
        }
      } catch (error) {
        ...
        <View style={styles.buttonContainer}>
          ...
          >
            {recording ? (
              ...
              <MaterialIcons name="fiber-manual-record" size={28} color="#FFF" />
            )}
          </TouchableOpacity>
          <TouchableOpacity
